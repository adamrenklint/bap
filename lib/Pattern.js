...
var Pattern = PlayableModel.extend({
  ...
  notes: function () {
    ...
    this.channels.each(function (channel) {
      notes.push.apply(notes, channel.expandedNotes.models);
    });
    return notes;
  },

  kit: function (id, kit) {
    if (!~this.kits.validIds.indexOf(id)) {
      throw new Error('Invalid argument: kit id must be single letter from A to Z');
    }
    else if (kit) {
      this.kits[id] = kit;
      return this;
