...
var Sequence = PlayableModel.extend({
  ...
  notes: function (returnOriginals, from, to) {
    var notes = [];
    var cids = [];
    var offset = 0;
    var cid = this.cid;
    console.log(cid, 'from:', from, 'to:', to);
    function add (sequence, length) {
      sequence.notes(returnOriginals).forEach(function (note) {
        if (returnOriginals) {
        ...
        // else if (offset < includes[0]) {
        else {
          var bar = note.bar + offset;
          // console.log(cid, bar, includes);
          // if (includes && !~includes.indexOf(bar)) {
          //   return;
          // }
    ...
    this.sequences.forEach(function (row) {
      ...
      console.log(offset, from, to);
      if (this._isOffsetWithinRange(offset, from, to)) {
        row.forEach(function (item) {
  ...
  _isOffsetWithinRange: function (offset, from, to) {
