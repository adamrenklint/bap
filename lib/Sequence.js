...
var expressions = require('../node_modules/dilla/node_modules/dilla-expressions');
...
var Sequence = PlayableModel.extend({
  ...
  notes: function (returnOriginals, includes) {
    var notes = [];
    var cids = [];
    var offset = 0;
    function add (row, index, sequence, longestInRow) {
      if (sequence.type === 'pattern') {
        // sequence.notes().
        sequence.notes().forEach(function (note) {
          // var clone = note.ghost();
          // var bar = note.bar && note.bar + offset;
          // var clone;
          // if (includes && bar && !~includes.indexOf(bar)) {
          //   return;
          // }
          // if (bar) {
          //   clone = note.ghost();
          ...
          // }
          // else {
          //   var parts = note.position.split('.');
          //   var data = Sequence.addOffsetToExpression(parts[0], offset, length);
          //   var reject = includes && !includes.some(function (n) {
          //     return n > data.gt && n < data.lt;
          //   });
          //   if (reject) {
          //     return;
          //   }
          ...
          // }
          // clone && notes.push(clone);
        });
        //expressions
      }
      else {
        // console.log(index, longestInRow);
        sequence.notes().forEach(function (note) {
          notes.push(note);
        });
      }
      ...
      */
      ...
      // sequence.notes(false, includes).forEach(function (note) {
      //   if (returnOriginals) {
    ...
    this.sequences.forEach(function (row) {
      ...
      row.forEach(function (sequence, index) {
        add(row, index, sequence, longestInRow);
      });
      offset += longestInRow;
    });
    return notes;
