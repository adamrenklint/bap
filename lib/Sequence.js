...
var Sequence = PlayableModel.extend({
  ...
  notes: function (bar, beat, tick) {
    ...
    notes[bar] = [];
    // how to calculate offset and get notes from bar?
    // - find the right sequence > pattern
    // this._patternsRelativeTo(bar) > pattern+?offsetBars
    //

    return notes;
  },

  // I HAVE TO DO THIS IN A WAY THATS OPTIMIZED FOR GETTING IT WITH FULL POSITION: bar, beat, tick FROM HEARTBEAT STEP

  _patternsRelativeTo: function (bar) {
    // returns a merged trie from all patterns, relative to bar
