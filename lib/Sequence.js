...
var Sequence = PlayableModel.extend({
  ...
  notes: function (bar, beat, tick) {
    ...
    Object.keys(patternsForBar).forEach(function (offset) {
      ...
      var innerBar = 1 - parseInt(offset, 10);
  ...
  patterns: function (bar, patterns) {
    ...
    next.forEach(function (seq) {
      if (seq.type === 'sequence') {
        var innerBar = bar - offset;
        if (innerBar <= seq.bars) {
          patterns = seq.patterns(bar - offset, patterns);
        }
      }
      else {
        var patternOffset = bar - offset - 1;
        patterns[patternOffset] = patterns[patternOffset] || [];
        patterns[patternOffset].push(seq);
      }
    });
