...
var Channel = Model.extend(volumeParams, {
  ...
  // runTransforms: if note is plain position, and there are no transforms, then still shouldn't create ghost!? we want to avoid creating ghosts!!!

  _onAddRawNote: function (note) {
    // var transform = this.transforms(note);

    if (note.hasPlainPosition()) {
      note.original = note;
