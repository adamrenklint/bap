...
var Channel = Model.extend(volumeParams, {
  ...
  notes: function (bar, beat, tick) {
    ...
    var beats = cache[bar];
    if (!beat) return beats['*'].slice();
    var ticks = beats[beat];
    if (!tick) return ticks['*'].slice();
    return ticks[tick].slice();
  },

  _onAddExpandedNote: function (note) {
    var cache = this._cache;
    cache[note.bar] = cache[note.bar] || {};
    cache[note.bar]['*'] = cache[note.bar]['*'] || [];
    cache[note.bar]['*'].push(note);
    cache[note.bar][note.beat] = cache[note.bar][note.beat] || {};
    cache[note.bar][note.beat]['*'] = cache[note.bar][note.beat]['*'] || [];
    cache[note.bar][note.beat]['*'].push(note);
    cache[note.bar][note.beat][note.tick] = cache[note.bar][note.beat][note.tick] || [];
    cache[note.bar][note.beat][note.tick].push(note);
  },

  _onRemoveExpandedNote: function (note) {
    var cache = this._cache;
    var barIndex = cache[note.bar]['*'].indexOf(note);
    cache[note.bar]['*'].splice(barIndex, 1);
    var beatIndex = cache[note.bar][note.beat]['*'].indexOf(note);
    cache[note.bar][note.beat]['*'].splice(beatIndex, 1);
    var tickIndex = cache[note.bar][note.beat][note.tick].indexOf(note);
    cache[note.bar][note.beat][note.tick].splice(tickIndex, 1);
  },
  ...
  _onRemoveRawNote: function (note) {
    this.expandedNotes.models.slice().forEach(function (ghost) {
      console.log('?', ghost.position, ghost.key, !!ghost.original, note === ghost.original)
