...
var Clock = PositionModel.extend({
  ...
  start: function (sequence) {
    ...
    if (!sequence && this.sequence && this.position !== '0.0.00') {
      ...
      // make sure we have the right tempo
      // this.engine.setTempo(this._findNextTempo());
    }
  },
  ...
  pause: function (sequence) {
    ...
    else if (!sequence || sequence === this.sequence) {
      this.engine.pause();
      this.playing = false;
      this.tempo = 0;
  ...
  // _scheduleTempo: function () {
  //   var tempoChanges = this.sequence.tempoChanges && this.sequence.tempoChanges();
  //   this.engine.set('tempo', tempoChanges || []);
  // },
  ...
  _onChangeSequence: function () {
    ...
    // this.listenTo(this.sequence, 'change:channels', this._scheduleTempo);
    // this.listenTo(this.sequence, 'change:sequences', this._scheduleTempo);
  ...
  _onChangePosition: function () {
    if (this.position !== '0.0.00' && this.position !== this.engine._position) {
      this.engine.setPosition(this.position);
      // console.log('set', this._findNextTempo())
      // this.engine.setTempo(this._findNextTempo());
    }
  },
  ...
  _onEngineStep: function (step) {
    ...
    else if (step.id === 'tempo') {
      // && (this.sequence.loop || step.args.position !== '1.1.01')
      this.engine.setTempo(step.args.val);
    }
  },

  // _findNextTempo: function () {
  //   var current = this.position;
  //   var tempoChanges = this.sequence && this.sequence.tempoChanges && this.sequence.tempoChanges().filter(function (change) {
  //     return change[0] <= current;
  //   });
  //   console.log(current, tempoChanges)
  //   // var tempoChanges = this.engine.get('tempo').filter(function (change) {
  //   //   return change.position <= current;
  //   // });
  //   var index = this.sequence.loop ? 1 : 2;
  //   var next = tempoChanges && tempoChanges[tempoChanges.length - index];
  //   return next && next.val || this.sequence && this.sequence.tempo || 120;
  // },
