...
var Clock = PositionModel.extend({
  ...
  _scheduleSteps: function () {
    ...
    this.engine.set('steps', steps.map(function (step) {
      return [step];
    }));
  },
  ...
  _onChangePosition: function () {
    if (this.position !== '0.0.00' && this.position !== this.engine._position) {
      console.log('_changePosition', this.position);
      this.engine.setPosition(this.position);
    }
  },

  // _onChangeBeat: function () {
  //   console.log('_onChangeBeat', this.bar, this.beat, this.tick);
  //   this._scheduleSteps();
  // },
  //
  ...
  _onEngineTick: function (tick) {
    // console.log('tick.position', tick.position);
