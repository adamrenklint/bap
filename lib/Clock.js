...
var Clock = PositionModel.extend({
  ...
  initialize: function () {
    ...
    this.on('change:bar', this._onChangeBar);

    this._schedule = debounce(this._schedule);
  ...
  _onChangeSequence: function () {
    ...
    this.listenTo(this.sequence, 'change:channels', this._schedule);
    this.listenTo(this.sequence, 'change:sequences', this._schedule);
  ...
  _onChangeBar: function () {
    console.log('_onChangeBar');
    if (this.playing) {
      this.sequence.detachGhosts();
      this._schedule();
    }
  },
  ...
  _schedule: function () {
    ...
    console.log('schedule\n=============', this.bar, sequence.bars);
    var nextBar = this.bar + 1;
    if (nextBar > sequence.bars) {
      nextBar = 1;
    }
    var includes = [this.bar, nextBar];

    var start = new Date();
    var notes = sequence.notes(false, includes).map(function (note) {
      console.log(note.position, note.key);
      return [note.position, note];
    });
    console.log('==', includes, notes.length)
