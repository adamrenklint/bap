...
var Clock = PositionModel.extend({
  ...
  start: function (sequence) {
    ...
    if (!sequence && this.sequence) {
      ...
      var tempo = next && next.val || this.sequence.tempo;
  ...
  copySequenceParams: function (sequence) {
  ...
  schedule: function (sequence, name) {
    this.copySequenceParams(sequence);
  ...
  onSchedulerTick: function (tick) {
    if (this.stopByFold && tick.position > '1.1.00') {
      ...
      this.tick = 96;
      // to reset the tempo and beatsPerBar to the start value
      // should not be needed with the new thing in start()
      // this.copySequenceParams(this.sequence);
    }
