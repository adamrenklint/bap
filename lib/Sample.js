...
Sample.getNewBufferSize = function (params, buffer) {
  var size = buffer.sampleRate * params.length;
  if (params.loop) {
    return buffer.sampleRate * params.loop;
  }
...
Sample.getNewBufferStartingPoint = function (params, buffer) {
  return buffer.sampleRate * params.offset;
};

Sample.makeBuffer = memoize(function (params, original, context) {

  var size = Sample.getNewBufferSize(params, original);
  var numberOfChannels = Sample.getNewBufferChannelCount(params, original);
  var sampleRate = original.sampleRate;
  var startingPoint = Sample.getNewBufferStartingPoint(params, original);

  var buffer = context.createBuffer(numberOfChannels, target, sampleRate);

  console.log(params.src, size, numberOfChannels);


  //   buffer = Sample.cutBufferToLength(buffer, params.length, context);
  // }
  //
