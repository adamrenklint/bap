...
Sample.cutBufferToLength = function (buffer, length, context) {
  ...
  var target = Math.floor(length * sampleRate);
  ...
  for (var i = 0; i < numberOfChannels; i++) {
    ...
    while (index > -1) {
      chunk = data[index];
      if (!didFindEnd) {
        // the number 300 here is sort of a "magic number" that fixes the clipping at the tail of compressor smoke tests and boombap example
        if (dismissed > 300 && lastChunk > 0 && chunk < 0) {
          didFindEnd = true;
        }
        else {
          chunk = 0;
