...
var Sample = Layer.extend(sampleParams, {
  ...
  makeBuffer: function (params, buffer) {
    if (!params.channel || buffer.numberOfChannels < 2) {
      return buffer;
    }
    var newBuffer = this.context.createBuffer(1, buffer.length, buffer.sampleRate);
  ...
  source: function (params) {
    ...
    source.buffer = this.makeBuffer(params, this.buffer);
